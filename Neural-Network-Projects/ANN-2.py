...
class Network:
    ...
    def feed_backward(self, a, data, i, learning_rate):
        ...
        for j in range(len(a[i])):
            ...
            b_gradient = learning_rate * b_error * self.sigmoid_prime(a[i][j])
            b_gradients[j] = b_gradient
            for m in range(len(self.weights[i - 1][j])):
                w_error = 2 * (a[i][j] - data[2 + j])  # only for the last layer
                w_gradient = learning_rate * w_error * self.sigmoid_prime(a[i][j]) * a[i - 1][m]
                w_gradients[j][m] = w_gradient
        gradients = (w_gradients, b_gradients)
        return gradients

    def backward_propagation(self, data, learning_rate):
        a = self.neuron_values
        if len(self.hidden_layer) == 0:  # if there is no hidden layer
            return self.backward_propagation_without_hidden_layer(a, data, learning_rate)
        else:
            return self.backward_propagation_with_hidden_layer(a, data, learning_rate)

    def backward_propagation_without_hidden_layer(self, a, data, learning_rate):
        return self.feed_backward(a, data, 1, learning_rate)

    def backward_propagation_with_hidden_layer(self, a, data, learning_rate):
        layer_gradients = []
        for i in range(len(self.hidden_layer) + 1, 0, -1):
            w_gradients, b_gradients = self.feed_backward(a, data, i, learning_rate)
            layer_gradients.append((w_gradients, b_gradients))
        return layer_gradients

    def update_weights_and_biases(self, data_gradients, n):
        a = len(self.neuron_values)
        for i in range(a - 1):
            w_matrix = np.zeros(self.weights[i].shape)
            for j in range(n):
                w_matrix += data_gradients[j][0][0][i]
            w_matrix = w_matrix / n
            self.weights[i] -= w_matrix

            b_matrix = np.zeros(self.biases[i].shape)
            for j in range(n):
                b_matrix += data_gradients[j][1][0][i]
            b_matrix = b_matrix / n
...
def main():
    ...
    for i in range(500):
        ...
        for data in train_data:
            ...
            layer_gradients = network.backward_propagation(data, learning_rate)
            data_gradients.append(layer_gradients)

        error.append(sum)

        network.update_weights_and_biases(data_gradients, len(train_data))
